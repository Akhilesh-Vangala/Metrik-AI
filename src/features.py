...
def add_lag_features(
    ...
    df["lag_diff_24h"] = (df.get("lag_24h", np.nan) - df.get("lag_168h", np.nan)).astype(np.float32)
...
def add_interaction_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "rolling_mean_24h" in df.columns and "lag_24h" in df.columns:
        mean_24 = df["rolling_mean_24h"]
        safe_mean = mean_24.replace(0, np.nan)
        df["reading_vs_rolling"] = (df["lag_24h"] / safe_mean).clip(-10, 10).astype(np.float32)
